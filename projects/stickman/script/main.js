...
function charMovement() {
 ...
 if (player.velx < 0 || groundMovement.velx > 0) {
	 //När spelar rör sig åt vänster körs koden som kollar ifall ett block är
	 //vänster om spelaren. Om det är det dvs om "walkable = false"
	 //stannar spelaren.
	   ...
	   var halfy = Math.floor((player.y + player.sizeY/2) / 50)
		 % 1000;
			 var bottomy = Math.floor((player.y + player.sizeY - 1) / 50)
			   % 1000;
   if (player.x <= 0) {player.x = 0; player.velx = 0;}//stoppar spelaren från att gå in utanför vänstra delen av banan
   if (x < 0 || bottomy == null || map[x][halfy] == null || map[x][y] == null ||
	  map[x][bottomy] == null) {}//kollar om spelaren är i grid-systemet eller inte. map[x][y] blir undefined om de inte är innanför banan.
   else if (map[x][halfy].walkable == false || map[x][y].walkable == false ||
	  map[x][bottomy].walkable == false) {
	 player.x = map[x][y].x + map[x][y].sizeX;//gör så att spelaren inte fastnar i blocket
	 groundMovement.x = groundMovement.x;
	 groundMovement.velx = 0;//stoppar hastigheten för spelaren
	 if (key.s == true) {
	   player.vely = 0;
	 }
  ...
  else if (player.velx > 0 || groundMovement.velx < 0) {//samma som ovan fast när spelaren går åt höger
   ...
   if (player.vely < 0 || groundMovement.vely > 0) {//när spelaren hoppar/åker uppåt körs detta
	 ...
	 var rightx = Math.floor((player.x + player.sizeX - groundMovement.x) / 50)
	   % 1000;
	   var nexty = Math.floor((player.y+player.vely) / 50)//hämtar nästa kordinat på y axeln så att programmet vet ifall kordinaten är i ett block innan det händer
		 % 1000;
	...
	var x = Math.floor((player.x - groundMovement.x + groundMovement.velx) / 50)
	  ...
	  var rightx = Math.floor((player.x + player.sizeX - groundMovement.x + groundMovement.velx) / 50)
		...
		if (player.y > canvas.height + player.sizeY) {
			player.hp = 0; //dödar spelaren om den hamnar under canvasen
		}
	  if (x < 0 || map[rightx][nextBottomy] == null || map[x][nextBottomy] == null) {}
	  else if (map[rightx][nextBottomy].walkable == false || map[x][nextBottomy].walkable == false) {//kollar ifall spelaren står på ett block
		player.vely = 0;																																							 //om inte kan man inte hoppa
		player.jump = false;
	  ...
	  if (player.crouch == true){
		...
		var rightx = Math.floor((player.x + player.sizeX - groundMovement.x) / 50)
		  ...
		  var nexty = Math.floor((player.y+player.vely-player.sizeY) / 50)
			% 1000;
			var bottomy = Math.floor((player.y+player.vely-player.sizeY) / 50)
			% 1000;
		if (map[x][nexty] == null || map[rightx][nexty] == null || map[rightx][bottomy] == null || map[x][bottomy] == null) {}
			...
			//kollar ifall det finns ett block ovanför, om den gör den kan man inte ställa sig upp ifall man är i crouch
		  player.standEnabled = false;
		}
		else {
		  player.standEnabled = true;
		}
		if (key.s == false && player.standEnabled == true) {//gör så att spelarn automatiskt ställer sig upp om det inte finns ett block ovanför
		  player.crouch = false;
		  playerCrouch();
		}
  ...
  if (player.velx > 0.01 && player.velx < -0.01) {player.x += player.velx;}//spelaren stannar helt om hastigheten är för långsam
  if (player.vely > 0.01 && player.vely < -0.01) {player.y += player.vely;}


  if (groundMovement.x < 0) {
	if (-groundMovement.x >= mapTileWidth*50-1203 && player.x >= 300) {//gör så att bara spelaren rör sig när man är vid slutet
	...
	if (groundMovement.x <= 0 && player.x >= 300) {//bara spelaren rör sig vi början av banan
	  groundMovement.velx += -player.velx;
	  player.velx = 0;
	  groundMovement.x += groundMovement.velx;
	  player.x = 300;
...
function mouseAim() {
	//ritar ut siktet på musens position
		...
		if (hitTile.block) {
			...
			if (map[x][y].hp <= 0 && map[x][y].image == wood) {
			  ...
			  genPieces(map[x][y].x+25, map[x][y].y+25, 3, woodPieces);
							 ...
							 map[x][y].walkable == false && map[x][y].image == metalDoorImg) {
				...
				genSplash(gun.hitX, gun.hitY, "brown");
							 ...
							 map[x][y].walkable == false && map[x][y].image == caveGroundImg ||
						 	 ...
						 	 map[x][y].walkable == false && map[x][y].image == caveGroundFlipImg) {
